export default function Errors({ errorStartingAmount = false, errorNoLetters = false }) {
  return (
    <>
      <section aria-live="assertive" aria-label={errorStartingAmount ? 'Error messages' : undefined}>
        {errorStartingAmount ? (
      ...
      <section aria-live="assertive" aria-label={errorNoLetters ? 'Error messages' : undefined}>
        {errorNoLetters ? (
